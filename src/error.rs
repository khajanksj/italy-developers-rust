use actix_web::{error::JsonPayloadError, http::StatusCode, web, HttpRequest, HttpResponse, ResponseError};
use thiserror::Error;

#[derive(Debug,Error)] pub enum AppError { #[error("not found")] NotFound, #[error("invalid request")] BadRequest, #[error("forbidden")] Forbidden, #[error("service unavailable")] Database(#[from] sqlx::Error), #[error("template error")] Template(#[from] askama::Error) }
