...
async fn services(db: web::Data<Database>) -> Result<HttpResponse, AppError> {
    collection_page(&db,"service","Technology Services for People and Businesses in Italy | Italy Developers","Websites, APIs, dashboards and responsible AI delivered exclusively for local businesses and individuals in Italy.","/services","Italy-only services","Technology for Italy, built by a global community.","We accept client work only from Italy. Developers may contribute from anywhere in the world.").await
}
